...
void main()
{
    ...
    //ÊäÈëŽý²âÍŒÆ¬
char* filename = "/home/kal-thir/qbbbbbb_yolo_fresh/pygen0/a.jpg";

    image im = load_image_stb(filename, 3);
    image sized = letterbox_image(im, 416, 416);

    FILE* fp0;
    fp0 = fopen("./inputimg.h", "w");   //Éú³ÉÍŒÆ¬ÊýŸÝŽæ·ÅÎ»ÖÃ
    fprintf(fp0, "#ifndef INPUT_IMAGE_H\n");
    fprintf(fp0, "#define INPUT_IMAGE_H\n\n");
    fprintf(fp0, "int im_w=%d;\n", im.w);
    fprintf(fp0, "int im_h=%d;\n", im.h);
    fprintf(fp0, "int sized_w=%d;\n", sized.w);
    fprintf(fp0, "int sized_h=%d;\n", sized.h);
    fprintf(fp0, "float SizedData[]={");
    int sized_w = sized.w;
    int sized_h = sized.h;
    int k;
    int j;
