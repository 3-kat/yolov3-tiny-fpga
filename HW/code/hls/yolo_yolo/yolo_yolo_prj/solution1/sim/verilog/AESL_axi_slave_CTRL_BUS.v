...
//------------------------Parameter----------------------
`define TV_IN_activate_en_V "../tv/cdatafile/c.yolo_yolo_top.autotvin_activate_en_V.dat"
`define TV_IN_input_h_V "../tv/cdatafile/c.yolo_yolo_top.autotvin_input_h_V.dat"
`define TV_IN_input_w_V "../tv/cdatafile/c.yolo_yolo_top.autotvin_input_w_V.dat"
parameter ADDR_WIDTH = 6;
parameter DATA_WIDTH = 32;
parameter activate_en_V_DEPTH = 1;
reg [31 : 0] activate_en_V_OPERATE_DEPTH = 0;
parameter activate_en_V_c_bitwidth = 32;
parameter input_h_V_DEPTH = 1;
reg [31 : 0] input_h_V_OPERATE_DEPTH = 0;
parameter input_h_V_c_bitwidth = 5;
parameter input_w_V_DEPTH = 1;
reg [31 : 0] input_w_V_OPERATE_DEPTH = 0;
parameter input_w_V_c_bitwidth = 5;
parameter START_ADDR = 0;
parameter yolo_yolo_top_continue_addr = 0;
parameter yolo_yolo_top_auto_start_addr = 0;
parameter activate_en_V_data_in_addr = 16;
parameter input_h_V_data_in_addr = 24;
parameter input_w_V_data_in_addr = 32;
...
reg  BREADY_reg = 0;
reg [DATA_WIDTH - 1 : 0] mem_activate_en_V [activate_en_V_DEPTH - 1 : 0];
reg activate_en_V_write_data_finish;
...
//write activate_en_V reg
reg [31 : 0] write_activate_en_V_count = 0;
reg write_activate_en_V_run_flag = 0;
reg write_one_activate_en_V_data_done = 0;
//write input_h_V reg
...
assign TRAN_CTRL_BUS_write_data_finish = 1 & activate_en_V_write_data_finish & input_h_V_write_data_finish & input_w_V_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        activate_en_V_write_data_finish <= 0;
        write_activate_en_V_run_flag <= 0; 
        write_activate_en_V_count = 0;
        count_operate_depth_by_bitwidth_and_depth (activate_en_V_c_bitwidth, activate_en_V_DEPTH, activate_en_V_OPERATE_DEPTH);
    end
    else begin
        ...
        if (AESL_ready_reg === 1) begin
            write_activate_en_V_run_flag <= 1; 
            write_activate_en_V_count = 0;
        end
        if (write_one_activate_en_V_data_done === 1) begin
            write_activate_en_V_count = write_activate_en_V_count + 1;
            if (write_activate_en_V_count == activate_en_V_OPERATE_DEPTH) begin
                write_activate_en_V_run_flag <= 0; 
                activate_en_V_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_activate_en_V
    integer write_activate_en_V_resp;
    integer process_num ;
    integer get_ack;
    ...
    integer i;
    integer j;
    reg [31 : 0] activate_en_V_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = activate_en_V_c_bitwidth;
    process_num = 1;
    ...
    while (1) begin
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_activate_en_V_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write activate_en_V data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (activate_en_V_c_bitwidth < 32) begin
                        activate_en_V_data_tmp_reg = mem_activate_en_V[write_activate_en_V_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < activate_en_V_c_bitwidth) begin
                                activate_en_V_data_tmp_reg[j] = mem_activate_en_V[write_activate_en_V_count][i*32 + j];
                            end
                            else begin
                                activate_en_V_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    write (activate_en_V_data_in_addr + write_activate_en_V_count * four_byte_num * 4 + i * 4, activate_en_V_data_tmp_reg, write_activate_en_V_resp);
                end
                process_busy = 0;
                write_one_activate_en_V_data_done <= 1;
                @(posedge clk);
                write_one_activate_en_V_data_done <= 0;
            end   
            process_1_finish <= 1;
        end
        @(posedge clk);
...
// Read data from file 
initial begin : read_activate_en_V_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [127 : 0] token; 
  reg [127 : 0] token_tmp; 
  //reg [activate_en_V_c_bitwidth - 1 : 0] token_tmp; 
  ...
  count_seperate_factor_by_bitwidth (activate_en_V_c_bitwidth , factor);
  fp = $fopen(`TV_IN_activate_en_V ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_activate_en_V); 
      $finish; 
  end 
  read_token(fp, token); 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < activate_en_V_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  ...
                  mem_activate_en_V [i/factor] = mem_tmp;
                  mem_tmp [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
